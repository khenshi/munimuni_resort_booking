...
export function readCustomerBookings() {
  try {
    const parsed = JSON.parse(window.localStorage.getItem(CUSTOMER_BOOKINGS_STORAGE_KEY) ?? '{}')
...
export function writeCustomerBookings(bookingsMap) {
  window.localStorage.setItem(CUSTOMER_BOOKINGS_STORAGE_KEY, JSON.stringify(bookingsMap))
...
export function updateCustomerBooking(customerId, bookingReference, updatedBooking) {
  if (!customerId) return
  const allBookings = readCustomerBookings()
  if (!allBookings[customerId]) {
    return
  }
  const index = allBookings[customerId].findIndex((b) => b.bookingReference === bookingReference)
  if (index !== -1) {
